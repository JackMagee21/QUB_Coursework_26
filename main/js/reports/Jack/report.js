...
const getAllStudentsWithoutAssistanceNoDups = async () => {
    const sql = `SELECT s.Student_ID, s.Forename, s.Surname FROM Student s LEFT JOIN Student_Disability_Support sds ON s.Student_ID = sds.Student_ID WHERE sds.Student_ID IS NULL`;
    const result = await runQuery(sql);
    return result;
}