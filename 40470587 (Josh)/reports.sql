...
SELECT ROUND(AVG(ts.Grade), 2) AS Grade, d.Disability_Type 
FROM Student s 
...
SELECT COUNT(s.Student_ID) AS numStudents, d.Disability_Type 
FROM Student s 
JOIN Student_Disability_Support sds ON sds.Student_ID = s.Student_ID 
...
SELECT t.Test_ID, t.Name, MIN(ts.Grade) AS minGrade, MAX(ts.Grade) AS maxGrade, AVG(ts.Grade) AS avgGrade 
FROM Test t 
